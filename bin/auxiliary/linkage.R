...
#' 
#' @param sink_plink
#' If TRUE, `sink(/dev/null)` will be called around the call to PLINK. Set to 
#' FALSE if plink_bin is NULL.
#' 
#' @param file_format
...
plink_clump <- function(
    ...
    if (sink_plink) {
        sink(file = "/dev/null", split = FALSE)
    }
...
modified_ld_clump_local <- function(dat, clump_kb, clump_r2, clump_p, bfile, plink_bin)
{
    ...
    fun2 <- paste0(
        ...
        " --clump-r2 ", clump_r2, 
        " --clump-kb ", clump_kb, 
        " --memory 15000",  # add memory flag so that it doesn't eat up memory
...
multi_chr_ld_matrix <- function(
    ...
    for (chr in as.integer(unique(x$chr.exposure))) {
        LD_chr <- ld_matrix_modified(x$SNP[x$chr.exposure == chr], chr,
                                     ...
                                     with_alleles=TRUE, plink_bin=plink_bin, sink_plink=sink_plink)
...
#' @export
#' @return data frame
ld_matrix_local_modified <- function(variants, bfile, plink_bin, with_alleles=FALSE) {
...
dat_to_MRInput_modified <- function(
    ...
    out <- plyr::dlply(dat, c("exposure", "outcome"), function(x) {
        ...
        if (get_correlations) {
            ...
            ld <- multi_chr_ld_matrix(x, linkage_file=linkage_file, plink_bin=plink_bin, sink_plink=sink_plink)
